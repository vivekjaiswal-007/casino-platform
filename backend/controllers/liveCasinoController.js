...
export const launchGame = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      token:         TOKEN,
      timestamp:     Date.now(),
      return:        `${process.env.SOFTAPI_RETURN_URL || 'http://localhost:5173/live-casino/return'}`,
      callback:      `${process.env.SOFTAPI_CALLBACK_URL || 'http://localhost:5000/api/live-casino/callback'}`,
      currency_code,
      language,
    }
